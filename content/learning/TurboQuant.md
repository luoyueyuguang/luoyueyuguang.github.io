...
因为对任何固定的单位向量 $x$，乘上随机旋转后，$\Pi\,x$ 会像“单位球面上随机取到的一个点”。论文用到的事实是：它的每个坐标服从下面这个分布：
...
`TurboQuant_prod` 目标是让内积估计无偏、低误差，而不追求让 $\hat{x}$ 本身最像 $x$。
...
| QJL 矩阵 | 显式存 $d\times d$ Gaussian 矩阵 | 工程上会考虑计算和存储优化 |
