...
把两者放到同一条轴上看，DD 的精度已经非常逼近 fp128；拉开差距的是**指数范围**。DD 被锁死在 double 的水平，只有 fp128 能把范围也放大。
...
double 只保留到第 14 位十进制（`14.39272672286` 之后开始分岔），相对误差 $5.1\times10^{-14}$，也就是 $10^6$ 项累加吃掉了约 2–3 位有效数字。同一段代码换成 `__float128` 累加，用 `%.34Qg` 打印出来是 `14.39272672286572363138112749318902`——和 50 位参考值对到约 33 位十进制。
...
对比一下三档精度在同一道题上的表现（都是 $H_{10^6}$ 的逐项累加，真值取 50 位十进制的 `14.39272672286572363138112749318858767664`）：double 停在 `14.392726722864989`（相对误差 $5.1\times10^{-14}$，约 13–14 位十进制），上面那段 `__float128` 对到约 33 位。double 丢掉的是 2–3 位而不是数量级级别的精度——这道题条件数温和，误差只按 $\sqrt{n}u$ 增长；fp128 的价值在于换一道 $\kappa$ 大得多的题时，它还有余量。
