聊 FP8，最常见的误解是"只有一种 FP8 格式"。但对 NVIDIA 张量核心来说，FP8 是一套**怎么用**的方法论：从 Hopper（H100）开始，张量核心用 **E4M3 + E5M2 两种 8 位格式的"混合配方"**，配合**逐张量动态缩放（amax → scale）**，在矩阵乘上拿到约 2 倍于 FP16 的吞吐，同时把训练精度保住。

> NVIDIA 的 FP8 是一套"**前向用更精的 E4M3，反向用范围更大的 E5M2，再用每个张量自己的 amax 算一个缩放因子把它们塞进 8 位**"的工程配方。格式本身的推导见 [[learning/precision/05-fp8|FP8]]；这里只看张量核心**怎么用它**。
...
- **E5M2**（bias = 15）：指数多一位、范围大，但只留 2 位尾数，适合**反向**的梯度：梯度需要更大的动态范围，对精度不那么敏感。

完整的格式推导、次正规数、以及"E4M3 为什么没有无穷"的编码细节，见 [[learning/precision/05-fp8|FP8]]。
...
两者的价值是**交换**关系：E4M3 精度高但对"值有多大"敏感，E5M2 范围大但精度更低。正是因为没有任何一种 8 位格式能同时守住精度和范围，训练时才要**两种混着用**。
...
单看精度，FP8 的单位舍入（E4M3 $2^{-4}$、E5M2 $2^{-3}$）比 FP16 的 $2^{-11}$ 差约两个数量级，所以它**不能替你做所有事**，只能做矩阵乘这块最重、最大量的活。
...
> 对照：**MXFP8** 把"一个张量一个 scale"改成"每 32 个连续元素一个 scale"，缩放因子用 8 位的 E8M0 存。scale 细粒度化后动态范围需求骤降，于是**所有值都能用更精的 E4M3**，不用再靠 E5M2 兜底。这是 [[learning/precision/07-mxfp8|MXFP8]] 的本质，也是 Blackwell 的新方向。
...
- **Rubin（Vera Rubin，Blackwell 的下一代）**：沿用 Blackwell 的全部精度（NVFP4、FP64、TF32、BF16、FP16、FP8/FP6、INT8，与 Blackwell 那张表完全一致），并**加上 Tensor Core 模拟（emulated）FP32/FP64**：把输入拆成低位分量、用低位宽张量核算，从而在低位宽核上逼近甚至超过原生 IEEE 精度。Rubin 与 Blackwell **向上兼容**，已优化的代码可直接迁移。注意 NVIDIA 对 Rubin 的定位是**增强的第五代 Tensor Core**（"enhanced fifth-generation"），而不是另起一代；增强的 Transformer Engine 提供 50 PFLOPS 的 NVFP4 推理吞吐。
- **存储格式 vs 纯计算**：FP8 作为存储格式能省显存/带宽（激活、权重、KV cache 都能按 FP8 存），**缩放因子本身是 FP32**（每个张量一个），与数据一起维护。MXFP8 则把 scale 换成 E8M0，随每 32 元素一块存，于是"存和算"是同一套块结构。
...
用 Transformer Engine 在 H100 上跑一个 FP8（HYBRID：前向 E4M3、反向 E5M2）的 Linear 层。**这段需要 CUDA GPU + PyTorch + transformer_engine**，站点里的浏览器 Python runner 只带 NumPy，跑不了，所以下面不贴运行输出：
...
第一行里 `amax = 3.5` 正好取到真实最大值，`3.5 / 0.0078125 = 448`，刚好顶到量程但没溢出；第二行把 `amax` 估成 `0.6`（真实最大值的 $0.6/3.5 \approx 1/5.8$），scale 同比例变小，`3.5` 和 `-0.6` 一起被裁到 $\pm 448$：这两个原本相差约 5.8 倍的数绝对值都顶到 448，相对大小彻底丢了，所以工程上宁肯 scale 偏大、不能偏小。
...
6. **别拿 FP8 打精度战**。它只留 1–2 位十进制有效数字给矩阵乘；需要更多有效位的地方（残差、归一化、softmax）留在 FP16/FP32。
