聊 4-bit 浮点，绕不开 **NVFP4**：NVIDIA Blackwell 上用于极大模型推理的"最小浮点"。它只有 4 个 bit（1 sign + 2 exponent + 1 mantissa），即**E2M1**，正数能表示的档位只有 8 个。可它正是 Blackwell tensor core 吞吐的顶点：位数砍到 4，换来的是**精度粗到只剩"几档"**，必须靠 block scaling 把这几档"换算"到实际数值附近，才勉强喂得动大模型。想先看清楚整个精度家族，见 [[learning/precision/01-overview|精度总览]]。
...
加上符号位，负数就是同样的这一列。所以 FP4 的**正数只有 8 档**（0 和 7 个非零档），任意实数都会被归一到离它最近的那一档，这是它"粗"的根源。它也没有 inf / NaN 编码，16 个 bit pattern 全是有限数。
...
**ULP 随量级翻倍。** E2M1 的尾数只有 1 位，在指数字段 $E \ge 1$ 的那一档里，两个相邻码值（$M=0$ 与 $M=1$）的间距是

$$
\text{ulp}(x) = 2^{(E-1) - 1} = 2^{E-2}
$$

$[1,2)$ 档（$E=1$）一格是 $0.5$，$[2,4)$ 档（$E=2$）是 $1$，$[4,8)$ 档（$E=3$）是 $2$。四舍五入最多偏半格，所以在 $1$ 附近 $u = \text{ulp}(1)/2 = 2^{-2} = 0.25$，正是表里那个值。

**一个具体的舍入例子。** 对 $x = 2.6$：它落在 $[2,4)$ 档，这一档只有 $2.0$ 与 $3.0$ 两个码值，取最近的 $3.0$，绝对误差 $0.4$（相对 $15.4\%$）。对 $x = 5.8 \in [4,8)$：码值是 $4.0$ 与 $6.0$，取 $6.0$，绝对误差 $0.2$（相对 $3.4\%$）。两者都是"偏了不到一档"，但量级越大相对误差越小：ULP 是绝对间隔，$u$ 才是相对尺度。
...
三者共同点是**基础元素都是 E2M1，有效精度和单位舍入完全相同**，差异在"这一档能放到多大/多细的位置上"。MXFP4 用 32 个元素共享一个 $2^n$ 粗缩放；[[learning/precision/03-mxfp4|NVFP4 的兄弟 MXFP4]] 的缩放是"跳档"的 2 的幂。NVFP4 把块缩到 16 个元素，且缩放开到 FP8 的 E4M3（支持非 2 的幂、更细的分数刻度），再配一个 per-tensor 的 FP32 二级缩放，所以同样 4-bit，NVFP4 比 MXFP4 更贴数据的动态范围。上一档的 [[learning/precision/05-fp8|FP8 E4M3]] 位宽翻倍、有效精度翻倍到 4 bit、$u$ 降到 0.0625，精度和值域都好得多，但显存和带宽成本也翻倍。

**块缩放把"值域"整体平移。** 裸 E2M1 的可表示集合只有 $\{\pm 0, \pm 0.5, \dots, \pm 6\}$，覆盖 $[0.5, 6]$。NVFP4 的实际值是 $x = s_{\text{tensor}} \cdot s_{\text{block}} \cdot P$，两级尺度都是乘性的：

- $s_{\text{block}}$ 是 E4M3，自身覆盖 $2^{-9} \sim 448$（约 17.8 个倍频），把每个 16 元素块的指数窗口平移到数据附近；
- $s_{\text{tensor}}$ 是 FP32，把整张量的量级再对齐一次。

于是单块的**绝对**范围是 $s_{\text{block}} \cdot [0.5, 6]$，整张量再乘 $s_{\text{tensor}}$，可以覆盖到接近 fp32 的量级；但无论怎么乘，E2M1 元素自身的**相对**分辨率 $u = 0.25$ 不变。块缩放换来的始终是范围，不是精度。

### 需要FP4的理由

需要（推理显存/带宽/吞吐是瓶颈时）：

1. **超大 LLM 的推理**。权重堆在 HBM 上，走内存带宽的钱比算得快还贵。NVFP4 把权重压到约 4.5 bit/值（4-bit 值 + 每 16 个值共享 1 个 FP8 scale + 每 tensor 1 个 FP32），比 [[learning/precision/09-fp16|FP16]] 省约 3.5× 显存、比 [[learning/precision/05-fp8|FP8]] 省约 1.8×，直接换来更高的 token 吞吐和更低的每 token 能耗（NVIDIA 称 GB300 NVL72 相对 Hopper 的每兆瓦吞吐提升约 5×、每用户 token 延迟改善约 10×，合计 AI factory 产出约 50×）。
2. **对单值精度不敏感的大模型**。用 PTQ / QAT 配合 per-block 缩放，像 DeepSeek-R1 这类模型从 FP8 降到 NVFP4，关键评测的精度损失能压到 1% 以内，这是 NVFP4 相对裸 FP4 的价值：靠更密的缩放把粗档位的误差分摊开。
3. **想要 Blackwell 的峰值算力**。FP4 是 Blackwell 上最小的数据类型，也就对应最高的 sparse/dense petaflops；在"算力优先、精度可容忍"的推理场景，它是压榨硬件选择。

需要小心的点

**数据有离群值或动态范围过大的层**。FP4 只有 8 档，一个 outlier 会把整块 scale 拉飞，其他小值全被压成 0 或噪声（这是 Clustering（聚类）/QQQ 要解决的问题）。此时应改 per-channel 或更高精度。

**精度 vs 范围 vs 成本**：FP4 三个维度全压到极限。有效精度最低（2 bit）、原生值域最窄（$0.5 \to 6$ 只有 3.6 个倍频，正规区 $1 \to 6$ 更只有 2.6 个；对照 fp16 的正规区有约 30 个倍频），换来的是 4-bit 的最小存储和最省的数据搬移，用正确性换吞吐的刻意选择。
...
- 预量化权重：Hugging Face 已有多款 NVFP4 checkpoint，如 `nvidia/DeepSeek-R1-0528-NVFP4`、`nvidia/Llama-3.1-405B-Instruct-NVFP4`、`black-forest-labs/FLUX.1-dev-NVFP4`。
...
真实输出（Python 3.12；与站点内置运行器的 Pyodide 0.26.4 + NumPy 1.26.4 一致）：

```text
FP4 (E2M1) magnitudes: [0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0]
  0.40 -> bits s=0 e=0 m=1  value=  0.50
  0.90 -> bits s=0 e=1 m=0  value=  1.00
  1.20 -> bits s=0 e=1 m=0  value=  1.00
  2.60 -> bits s=0 e=2 m=1  value=  3.00
  5.80 -> bits s=0 e=3 m=1  value=  6.00
  7.00 -> bits s=0 e=3 m=1  value=  6.00
 -0.50 -> bits s=1 e=0 m=1  value= -0.50
 -4.10 -> bits s=1 e=3 m=0  value= -4.00
```

输出展示了 4-bit 的"粗"：`0.4` 被吸到 `0.5`，`1.2` 被吸到 `1.0`，`2.6` 被吸到 `3.0`，相邻档位之间隔着 0.5 或更大的缝。这还没算 NVFP4 的 block scale：真实推理里，先把每 16 个值除以各自的 E4M3 缩放因子，再进这个量化函数，最后在反量化时乘回去。实际工程中，就近取整（round-to-nearest）通常带权重/激活的 per-block 或 per-channel 缩放，并配合 outlier 处理（如 QQQ）来控制误差。

### 一些 tips

1. **别用"4-bit"去估精度**。有效精度是尾数 + 1 = 2 bit，约 0.6 位十进制；拿总位宽 4 去算会高估一倍（1.2），很容易混淆的坑。
...
5. **训练别盲目上 4-bit**。4-bit 主要兑现的是推理的显存与带宽收益；训练的正反向与梯度累加通常还是 bf16/fp16。不过 NVIDIA 在 [NVFP4 预训练论文](https://arxiv.org/abs/2509.25149) 里证明 4-bit 训练并非不可能：用 **Random Hadamard transform 压平块级离群值 + 前后向同一的二维量化 + 随机舍入（无偏梯度）+ 选择性高精度层**，把 12B 模型在 10T token 上训到与 FP8 baseline 相当的训练损失与下游精度。但要付出这些稳定化手段的代价，裸上 4-bit 仍会崩。
