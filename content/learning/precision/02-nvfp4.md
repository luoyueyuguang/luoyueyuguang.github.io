...
**一个具体的舍入例子。** 对 $x = 2.6$：它落在 $[2,4)$ 档，这一档只有 $2.0$ 与 $3.0$ 两个码值，取最近的 $3.0$，绝对误差 $0.4$（相对 $15.4\%$）。对 $x = 5.8 \in [4,8)$：码值是 $4.0$ 与 $6.0$，取 $6.0$，绝对误差 $0.2$（相对 $3.4\%$）。两者都是"偏了不到一档"，但量级越大相对误差越小——这正是 ULP 是绝对间隔、$u$ 才是相对尺度的意思。
