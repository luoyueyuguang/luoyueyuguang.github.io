...
每个输出元素 $C_{ij}=\sum_k A_{ik}B_{kj}$ 的量化误差，一阶展开就是
...
\rho=\frac{\sqrt{\sum_k(A_{ik}B_{kj})^2}}{|C_{ij}|}
$$
...
注意实验算的是**总相对误差** $\|C_q-C\|/\|C\|$（弗罗贝尼乌斯范数），它反映的是**大多数条件良好的输出**（$\rho$ 平均 $\approx 1$），所以与 $K$ 无关。它不能说明每个元素都安全：**近消去的那一小部分元素 $\rho$ 很大，相对误差远高于 0.4%**。真正让误差放大的是两类 $\rho$ 来源：
...
- **相对误差约 $u\cdot\rho$**。压 $u$：更深位宽、更细的缩放（per-channel / per-block）；压 $\rho$：消除离群值、约束动态范围、避免近消去。
- **$K$ 不直接放大相对误差**，只在 $\rho$ 变大的情形（近正交 / 近消去）间接起作用。真正随 $K$ 累积误差的是**低精度累加**（见下一节）。
- **累加用 fp32** 把"累加舍入"消掉后，剩下的才是输入量化的 $u\cdot\rho$ 项。
- **分层量化 / 块缩放**压的是 $\rho$ 里"被全局 outlier 拖累"的部分，不能消掉 $u$ 本身。
