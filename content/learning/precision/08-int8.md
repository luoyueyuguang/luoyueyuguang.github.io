前面聊的 [[learning/precision/09-fp16|fp16]]、[[learning/precision/10-bf16|bf16]]、[[learning/precision/05-fp8|fp8]] 都是浮点：值在一条随指数变化的"对数刻度"上，动态范围大。**INT8 完全反过来**：它是一段**固定、均匀的网格**，256 个格子等距排开，没有指数、没有尾数、没有动态范围。它出现在精度话题里，是因为它是**量化的主力格式**：把连续浮点值压缩到 8 个比特的整数码点上，换来一倍到数倍的吞吐和存储。 返回 [[learning/precision/01-overview|精度总览]]。

> INT8 是一个均匀的"模子"。浮点数的网格是越靠近 0 越密（对数刻度），INT8 是**全程等距**：每个格子都一样宽。所以它的精度是"网格宽度除以值本身"：值越小，相对误差越大。量化的一切麻烦（校准、裁剪、per-channel scale）都是在想办法让这个"固定的模子"贴合数据实际占据的范围。

### 位布局

INT8 是**整型**。8 位两补码（two's complement）有符号整型：

| 字段 | 位数 |
| --- | --- |
| sign（MSB） | 1 |
| 数值位（低 7 位 $b_6 \ldots b_0$） | 7 |
| 总计 | 8 |
...
因为 INT8 没有指数，所谓"精度"要换一种说法：它指的是"网格被切成多少块、每块多宽"。
...
上式就是 INT8 的**核心公式**：相对误差由 **$\frac{R}{|x|}$（反过来就是动态范围）** 决定。同样的量程下，$|x|$ 越小相对误差越大；想让每个值都拿到 $0.2\%$ 级别的相对误差，就必须让 $R$ 收紧到接近值的实际范围。**校准**（calibration）做的就是这件事。这与浮点"$u$ 固定、相对误差恒定"截然不同。

### 与其他格式对比

对比分两个方向：**同为 8 位**的浮点（[[learning/precision/05-fp8|fp8]]、[[learning/precision/10-bf16|bf16]]），以及**更宽/更窄**的参照。
...
INT8 和 bf16 的"分辨率"相同：INT8 的 256 电平约等于 2.4 位十进制，bf16 的 8 位有效数字也是 2.4 位。差别在**动态范围**：bf16 的"0.39%" 相对误差在整个 $10^{38}$ 范围内都成立，INT8 的"0.196%" 相对误差只在一个**被校准过的窄量程**内成立；一旦量程用错（比如数据里混进一个大离群值），INT8 的相对误差会立刻恶化到几十个百分点，而 bf16 不受影响。**INT8 用动态范围换速度与体积。**
...
- **主流框架**：
  ...
  - TensorRT：INT8 PTQ（calibration）、SmoothQuant，以及 LLM 的 int8 加速；Intel 的 ONNX Runtime 也提供 int8 量化工具链。
...
1. **量程是命根子**。INT8 的精度完全取决于 $s$ 抓得多准；不校准直接硬量化，误差常常在几个百分点以上，校准后能压到 0.5% 以内。先看数据的动态范围再决定格式。
...
```text
== 非对称量化（unsigned [0,255]）==
量化码点: [255, 0, 114, 67, 81, 206]
scale = 0.0024, zero-point = 81
反量化: [0.41145882352941177, -0.19154117647058821, 0.07803529411764705, -0.03310588235294117, 0.0, 0.29558823529411765]
最大绝对误差 = 0.0006（理论上限 s/2 ≈ 0.0012）

== 对称量化（signed, z=0）==
scale = absmax/127 = 0.0032
码点: [127, -59, 24, -10, 0, 91], 反量化: [0.412, -0.1914015748031496, 0.07785826771653542, -0.03244094488188976, 0.0, 0.2952125984251968]

== 一个离群值摧毁整层 ==
absmax 被 295 撑到 scale = 2.3228
小值 0.078 量化后相对误差 = 100.0%

== per-channel scale 救回精度 ==
0.078 在 per-channel 下: q=24, 反量化=0.0779
```
...
运行这段 Python 会看到：正常权重经校准后最大绝对误差是 `0.0006`，落在 `s/2 ≈ 0.0012` 的理论上限内；而混入一个 `295` 的离群值后，`0.078` 这种小值的相对误差直接变成 `100.0%`：它的码点被舍入到 0，信息完全丢失。
