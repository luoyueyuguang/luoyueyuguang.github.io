...
但直接用 INT8 做科学计算显然不行：INT8 只有 7 位有效数字（约 2 位十进制），误差不可接受。**Ozaki scheme 的洞察是：这些 INT8 单元之所以"不准"，不是因为它把乘法做错了，而是因为我们在喂它之前就把数压成了低精度。如果喂给它的本身就是"无损切好的整片"，那么每个 INT8 乘法累加都精确无误。** 精度损失被转移到"最后一步按位权合成"——这一步用目标精度（比如 FP64）来做，误差可控。
...
一句话：**Ozaki 是把"硬件不擅长的那档精度"搬到"硬件擅长的那档计算"上。要么原生精度很弱（消费级 GPU），要么目标精度很高（超 FP64），否则得不偿失。**
...
下面用 Python + NumPy 给出**概念级的忠实实现**：按行取共享指数，把矩阵切成 INT8 片，用 `np.matmul`（int32 累加）模拟 INT8 张量核，再按位权合成。你会看到它复现了 Ozaki 的核心结构，且精度逼近高精度参考。
