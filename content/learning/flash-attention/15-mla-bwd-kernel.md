[[learning/flash-attention/13-mla|MLA 前向]] 讲了算法和 absorbed 思路；MLA 反向**复用了普通 attention 的梯度链**（重算 $ S $、$ P = e^{S-L} $、$ dS = P \circ (dP - D) $），**但 FA 仓库把它拆成了三个 kernel**，因为 $ dS $ 是共享中间量，而 dQ、dK、dV 各自要配不同的 B 操作数、归约维也不同。

仓库里这条 MLA 反向线是 **top-k 稀疏（DSA 式）的 absorbed MLA 反向**：入口 `_flash_attn_bwd_sparse_mla` 断言 `gather_kv_indices is not None`（"require gather kv indices for backward"），主核类名就叫 `FlashAttentionSparseMLABackwardSm100`。**FA 仓库的 MLA 训练路径是"latent + 稀疏 gather"，没有全稠密 MLA 反向**；DeepSeek 自己维护的 [[learning/flash-attention/13-mla|FlashMLA]] 是同一思路的另一套实现，两者别混。
...
`flash_attn/cute/` 里 MLA 反向是这三个（主核与 dQ/dQv 核的文件头署名 Colfax International，dK 核改写自 CUTLASS 示例、保留 NVIDIA 的版权行）：
...
这一路的累加必须是原子的：$ dK $ 的位置是"top-k 选中的 KV 行"，同一行 KV 会被很多 (query, head) 对选中（MQA 下 128 个 head 共享同一份 KV），每个贡献都要累加到同一个 `dK[seqlen_k]`。所以 epilogue 走的是按索引散射 + 原子加，并跳过 `-1` 哨兵（padding 的无效 top-k 槽位）：
...
- DeepSeek-V3.2-Exp / FlashMLA（另一套 DSA 稀疏核实现，与 FA 仓库这条线不同）：<https://github.com/deepseek-ai/FlashMLA>
