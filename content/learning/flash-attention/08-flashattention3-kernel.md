...
6. `gemm(... tiled_mma_pv, tOrP/tOsP, tOrV ...)`：**GEMM1**，$ O \mathrel{+}= P_{j-1} V_{j-1} $。注意它用的 P 和 V 都**慢半拍**：`tOrP` 是上一轮 softmax 算出的 $ P_{j-1} $，`tOrV` 用 `smem_pipe_read_v`（在 `++smem_pipe_read` 之前捕获的那个阶段，即上一块的 V）。所以它和本轮 GEMM0 的 $ Q K_j^\top $ 之间**没有任何数据依赖**，两个 GEMM 可以一起排在 tensor core 队列里，这正是算法篇的 2 级流水线：本轮只算 $ S_j $，上一块的 $ P V $ 挪到本轮来算。
7. `warpgroup_wait<1>()`：此刻有两个未完成的 WGMMA（GEMM0、GEMM1），`wait<1>` 等到手上只剩一个，即**先发出去的 GEMM0 已经算完**，所以下一步能安全释放 K。GEMM1 这时还在跑，正好被下面的 softmax 盖住；它的收尾在 `online_softmax` 之后（`warpgroup_wait<0>()`，然后才 `consumer_release` 掉那块 V）。
...
11. `online_softmax(...)`：对 $ S $ 做 $ e^{S - m} $（`flash::scale_apply_exp2`，用 `exp2f` 合成 `ffma`），并把结果累加进行和 `row_sum`（$ \ell $）。它**不管 $ O $ 的重缩**：`max_get_scale` 已经把旧 `row_sum` 乘过 rescale 因子，而 $ O $ 的重缩由第 14 步的 `rescale_o` 单独负责（FA2 的 `softmax_rescale_o` 是把这三件事揉在一起的，FA3 拆开了）。
...
**为什么这样就能重叠**：GEMM1 要的 $ P_{j-1} $ 和 $ V_{j-1} $ 都是上一轮就算好 / 搬好的，所以它和本轮 GEMM0 的 $ Q K_j^\top $ 之间没有依赖，两个 WGMMA 会先后进入 tensor core 队列；`warpgroup_wait<1>` 只等 GEMM0，于是这段 softmax（读 $ S_j $、算 $ e^{S - m} $、重缩 $ O $）**跑在 GEMM1 还在算的时候**，exp 单元和 tensor core 同时忙。softmax 出 $ P_j $ 之后，它又成为下一轮 GEMM1 的输入，就这样一块一块错位下去。再叠上 pingpong，另一个 warpgroup 的 GEMM 也会落在同一时间窗口里，整个 CTA 的 tensor core 基本不空。
...
FA3 Hopper 前向逐行读下来，就是**"把两个 WGMMA 和一次 softmax 交错成一个 `fwd_step`"**：GEMM0 算 $ S_j $（异步），GEMM1 用上轮 $ P $ 算 $ P V $（异步），中间的空档给 softmax 的 exp 用。TMA 在 producer 端持续喂 K/V，`pipeline` barrier 控制 stage 复用。一个 warpgroup 里因此几乎没有空等张量核的时刻。
