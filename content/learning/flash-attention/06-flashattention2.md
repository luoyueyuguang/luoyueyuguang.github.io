FA1 解决的问题是"HBM 读写太多"，FA2 解决的问题是"算力利用率太低"。A100 上 FA1 的 forward 只能到 30–50% 的理论峰值，backward 只有 25–35%（论文正文按 forward/backward 分开给；摘要里给的是一个更宽的整体区间 25–40%）。拿来对照的"GEMM 水平"是 80–90%，FA2 论文就是拿这个数当 A100 上的参照（同一句里说"optimized GEMM can reach up to 80–90% of the theoretical maximum device throughput"；FA3 又在 H100 上复用同一个区间：FA2 只到 35%，而优化过的 GEMM 能到 80–90%）。FA2 的输出和 FA1 一致，换的是**怎么在 GPU 上切分工作**，外加两处省掉非 matmul FLOPs 的算法微调。

> **FA2 的三个改进都可以归为"把时间花在 matmul 上，别花在非 matmul 和共享内存搬移上"。先用一个数字感受成本不对称：A100 的 FP16 matmul 峰值是 312 TFLOPs/s，而非 matmul 的 FP32 只有 19.5 TFLOPs/s。一个非 matmul FLOP 的成本约等于 16 个 matmul FLOP。**
...
FA2 维护"未归一化"的 $ \widetilde{O} $，每步只按 max 变化重缩（$ e^{m^{old}-m^{new}} $，不是它的逆；新块 max 变大时旧项要**缩小**到同一指数基准）：
...
> **论文这两处都印反了，别照抄。** (1) §3.1.1 的 $ \widetilde{O}^{(2)} = \mathrm{diag}(\ell^{(1)})^{-1}O^{(1)} + \dots $：$ O^{(1)} $ 是 FA1 那种**已归一化**的输出（$ O^{(1)} = \mathrm{diag}(\ell^{(1)})^{-1}e^{S^{(1)}-m^{(1)}}V^{(1)} $），还原成未归一化要**乘** $ \ell^{(1)} $（写成 $ \mathrm{diag}(\ell^{(1)}) $），取逆等于多除一次 $ \ell^{(1)} $。(2) Algorithm 1 第 10 行印的是 $ \mathrm{diag}(e^{m^{(j-1)}-m^{(j)}})^{-1}O^{(j-1)} $，但同一页的 $ \ell $ 行写的是 $ \ell^{(2)} = e^{m^{(1)}-m^{(2)}}\ell^{(1)} + \dots $（FA3 Algorithm 1 第 19/21 行也是同一处乘、一处取逆）：$ \ell $ 和 $ O $ 做的是同一件事，就是把统计量从旧 max 换到新 max 的指数基准上，因子必须同向，所以旧项要**乘** $ e^{m^{old}-m^{new}} $（新块 max 变大，旧项贡献就该缩小）。FA1 Algorithm 1 第 12 行写的就是这个形式：$ \mathrm{diag}(\ell^{new})^{-1}\big(\mathrm{diag}(\ell^{old})e^{m^{old}-m^{new}}O + e^{\tilde m - m^{new}}\tilde P V\big) $。
>
...
FA1 的**实现**（v1.x 的 `fmha_fprop_kernel_1xN.h`）已经在 forward 结尾算 `p_sum_log = p_max + __logf(sum)` 并存进 `softmax_lse` 这一张表，反向直接拿它做 `scale_apply_exp`。所以"只存 LSE"在实现层面 FA1 就已经这么干了；FA2 的贡献是把它写成算法约定（FA2 论文 Algorithm 1 第 13、15 行）并推广：有了 $ L $ 就不用为了重算 $ P $ 再引入 $ m/\ell $ 的分块归约。
...
代码层面，warp 分工来自 `TiledMma` 的 warp 布局（`Layout<Shape<Int<kNWarps>,_1,_1>>` 把 mma 的 M 维切给各 warp），每个 warp 分到自己那行块 $ Q $ 的 A 片段 `tSrQ`。代码里另有一个 `Is_Q_in_regs` 开关，打开后 $ Q $ 常驻寄存器（就是 [[learning/flash-attention/04-forward-kernel|forward kernel]] 里那个 `if (Is_Q_in_regs) ... tSrQ_copy_view` 分支），不过启动模板当前的实例化传的都是 `false`。

**split-K 真正贵在哪。** 除了多一次归约，还有两笔账：
...
**代价**：$ B_r $ 不能太大。寄存器用量随 $ B_r $ 线性涨，块一大就 spill；shared memory 里 $ Q/K/V $ 的几份缓冲也会先装不下。两种情况都让 kernel 要么掉速要么直接跑不起来。FA2 实测块大小常取 $ \{64, 128\} \times \{64, 128\} $，按 head dim 和设备共享内存调。
...
FA2 原生支持 MQA（multi-query attention）和 GQA（grouped-query attention）：多个 query head 共享一组 KV head。forward 里不复制 $ K, V $，而是把 KV 的 head 索引写成 `bidh / params.h_h_k_ratio`（`flash_api.cpp` 里 `params.h_h_k_ratio = h / h_k`，就是 group 大小 $ H_q / H_{kv} $），所以同一组 query head 会命中同一块 K/V。

backward 稍绕一点：内核仍然是"一个 query head 一个 block"，于是 host 侧先按 $ H_q $ 分配 `dk_expanded` / `dv_expanded`，让每个 query head 把自己的 $ dK, dV $ 写进对应位置，**再在 kernel 之外**用 `at::sum_out(dk, at::reshape(dk_expanded, {b, s, H_{kv}, H_q/H_{kv}, d}), {3})` 把 group 维加起来，得到 $ H_{kv} $ 份 $ dK, dV $。GQA 的反向求和在 kernel 之外做，代价是一次额外的内存往返。
