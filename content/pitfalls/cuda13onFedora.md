...
glibc 的 `<math.h>` 用 `noexcept(true)` 声明 `rsqrt`/`rsqrtf`，而 CUDA 13 的 `crt/math_functions.h` 没加，两者冲突；glibc 侧把 `rsqrt`/`rsqrtf` 收进了标准库声明，CUDA 头文件还停在旧签名上（CUDA 13 的宿主 GCC 支持区间是 6.x–15.x，GCC 15 在范围内，这不是"CUDA 还没支持 GCC 15"的问题）。
...
- [NVIDIA 开发者论坛：Fedora 43 与 CUDA 13.1 的 rsqrt/rsqrtf 报错](https://forums.developer.nvidia.com/t/fedora-43-and-nvcc-cuda13-1-error-exception-specification-is-incompatible-rsqrt-rsqrtf/354510) — 报错文本与头文件补丁的出处；主帖把起因归到 glibc 的 `noexcept(true)` 声明与 CUDA 声明不符，回帖确认已在 CUDA 13.2 修复。
